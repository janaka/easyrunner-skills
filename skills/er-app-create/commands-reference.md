...
| `--custom-domain` | string | none | Public domain the app is served at. Triggers DNS setup via the linked Cloudflare account when present. |
...
- Compose-file contents are validated locally with Podman's compose loader
  before the app is saved; a parse error prints the underlying error.
- If `--custom-domain` is set, the CLI looks up the linked Cloudflare account,
  creates or confirms an A record pointing at the server's IP, and prints the
  records it found. Failures are warnings (the app is still added).
...
```bash
# flow_a — build from a GitHub repo, no custom domain yet
er app add marketing-site my-vps https://github.com/me/marketing-site.git

# flow_a — with custom domain and a non-main branch
er app add marketing-site my-vps https://github.com/me/marketing-site.git \
...
er app add openclaw my-vps \
  ...
  --custom-domain openclaw.example.com
